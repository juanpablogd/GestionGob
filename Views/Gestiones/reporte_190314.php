...
class PDF extends PDF_Rotate
{
			...
			if($fila["id_convenio"]!=""){ //2) Convenio Derivado	1) Convenio Marco
				$sql = "WITH ft AS (
					...
					while ($row = pg_fetch_assoc($res)){
						...
						$this->Ln();
						$this->SetFillColor(234,239,247);$this->SetTextColor(3,35,61);
						$this->Cell(200,6,utf8_decode("Vr. fuentes: ".$row["fuentes"] ),1,null,"L");
						$this->Cell(70,6,utf8_decode("Vr. adición: ".$row["vr_adicion"] ),1,null,"L");
						$this->SetFont('Arial','B',9);
						$this->Cell(70,6,utf8_decode("Total: ".$row["vr_total"] ),1,null,"L");
	...
	{	
		...
		foreach($fotos as $key => $value) {
			...
			$urlSaga = '../../../../SIG/'.trim($propiedad[0]); //echo $urlSaga."************";
			$this->Image($urlSaga,$miX,40,100);
...
$pdf = new PDF();
//$pdf->pagInicio();
//$pdf->convenioResumen($id);
//$pdf->gestionResumen($idGestion);
$pdf->consulta($cx,$id);
$pdf->Output();
?>
